function updatePKSsystemAfterChangedAnswer

global questionModelMap KPcontentMap modelParameters nChannels
global grv lrv

pos=find((grv.studentHistory.product_area==(grv.product_area)) & (grv.studentHistory.dataSetNumber>0));
pos2=find(grv.studentHistory.areaPos(pos)==1);
posToUse=pos(pos2(end)):pos(end);
question_index=grv.studentHistory.question_index(posToUse);
correctness=grv.studentHistory.correctness(posToUse);
is_testQ=grv.studentHistory.is_testQ(posToUse);
...
batchQindex=question_index;
batchQcorrectness=correctness;
batchQisTest=is_testQ;
...
grv.studentHistory.PKSbeforeQ(posToUse)=PKSbeforeQ;
