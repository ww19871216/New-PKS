...
product = getProductForSimulation;
...
[KPcontentMap,questionModelMap,adaptiveTestParameters,initialAdaptiveTestSettings,masteryCutoffs,...
 modelParameters,nChannels,outsideQuestionsToTest,finalExamQuestions,PKSserviceVersion,contentLevelsTerminology]=downloadModelVariables(product);

[grv_fn,lrv_fn,grv,lrv]=initializeStudentIfNewAndGetStudentDetails(product,student_id,class_id,PKSserviceVersion);

computationShellFileFolder=product.longNames{2};
productStudentFolder=product.longNames{3};
computationShellFileKey=product.longNames{4};
...
while loggedOut==0
    ...
    if grv.product_area==51
        if grv.areaPos==0
            getAllKnowledgeHistoriesForClass(class_id,product,productStudentFolder);
        end
...
if answer==1
    ...
    writetable(sht,['ADAPTIVEstudentHistory_' computationShellFileKey '.csv']);
    ...
    PKSmodelAdaptiveShellFolderLocation=keyPaths.adaptiveShellFolder;
    if answer==1
        computationShellFolderLocation=keyPaths.computationShellFilesFolder;
        copyfile([pwd pathsep 'ADAPTIVEstudentHistory_' computationShellFileKey '.csv'], [computationShellFolderLocation computationShellFileFolder])
    end
    
    if strcmp(PKSmodelAdaptiveShellFolderLocation,pwd)==0
       movefile([pwd pathsep 'ADAPTIVEstudentHistory_' computationShellFileKey '.csv'], [PKSmodelAdaptiveShellFolderLocation pathsep])      
    end
    ...
    RerunStudentDataFolderLocation=[startPath 'MATLAB\PKS Model\Implementation\Rerun Adaptive Test Shell'];
    if answer==1
        ...
        copyfile([PKSmodelAdaptiveShellFolderLocation '/ADAPTIVEstudentHistory_' computationShellFileKey '.csv'], rerunAdaptiveTestShellFolderLocation)
    end
    %}
end
